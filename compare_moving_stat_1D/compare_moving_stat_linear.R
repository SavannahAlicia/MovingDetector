...
# streamdf <- data.frame(x = 5 * c(trackxmin, 
#                   trackxmin+tracksteplength,
#                   c(trackxmin+tracksteplength + diaglength*1:10),
#                   rep(trackxmin+tracksteplength + diaglength*10, 3),
#                   trackxmin+tracksteplength + diaglength*9:1,
#                   rep(trackxmin+tracksteplength + diaglength*1, 2),
#                   c(trackxmin+tracksteplength + diaglength*1:10),
#                   rep(trackxmin+tracksteplength + diaglength*10, 3),
#                   trackxmin+tracksteplength + diaglength*9:1,
#                   rep(trackxmin+tracksteplength + diaglength*1, 2),
...
#                   trackxmin+tracksteplength + diaglength*9:1,
#                   rep(trackxmin+tracksteplength + diaglength*1, 2),
#                   c(trackxmin+tracksteplength + diaglength*1:7)
# ),
# y = c(0, 
#       0, 
#       diaglength*1:10,
#       diaglength*10 + tracksteplength*1:3,
#       diaglength*9:1 + tracksteplength*3,
#       diaglength*1 + tracksteplength*4:5,
#       diaglength*1:10 + tracksteplength*6,
#       diaglength*10 + tracksteplength*7:9,
#       diaglength*9:1 + tracksteplength*9,
#       diaglength*1 + tracksteplength*10:11,
#       diaglength*1:10 + tracksteplength*12,
#       diaglength*10 + tracksteplength*13:15,
#       diaglength*9:1 + tracksteplength*15,
#       diaglength*1 + tracksteplength*16:17,
#       diaglength*1:7 + tracksteplength*18
# ))
swlen = 90
...
#example population
expop <- secrlinear::sim.linearpopn(mask = mesh, D = D_mesh_q)
excapthist <- sim_capthist(pop = expop,
                           traps, tracksdf, lambda0, sigma,
                           D_mesh_q, hazdenom, report_probseenxk = F)
...
sim_fit <- function(tracksdf, 
  ...
  if (Dmod == "~1"){
    ...
    stat_nll <- function(v){
      lambda0_ <- exp(v[1])#invlogit(v[1])
      sigma_ <- exp(v[2])
      D_mesh_ <- exp(v[5]) *exp(v[3]*(mesh_mat[,1] + v[4])^2)
      out <- negloglikelihood_stationary_cpp(lambda0_, sigma_,
                                             hazdenom, D_mesh_, 
                                             capthist, useall,
                                             dist_trapmesh, mesh_mat,
    ...
    nll <- function(v){
      lambda0_ <- exp(v[1])#invlogit(v[1])
      sigma_ <- exp(v[2])
      D_mesh_ <- exp(v[5]) * exp(v[3]*(mesh_mat[,1] + v[4])^2)#exp(beta1*(mesh$x + beta2)^2)
      out <- negloglikelihood_moving_cpp(lambda0_, sigma_,  
                                         hazdenom, D_mesh_,
                                         capthist, useall,
                                         induse, dist_trapmesh, mesh_mat,
    ...
    start <- c(#logit(lambda0),
      log(lambda0),
      log(sigma), beta1, beta2, log(beta3))
  }  
...
fit2 <- sim_fit(tracksdf, 
                ...
                dist_trapmesh,
                useall,
                lambda0, sigma, D_mesh_q, beta1, beta2, beta3,
                hazdenom, 
                mesh, Dmod = "~x^2")
...
times <- do.call(rbind, lapply(as.list(1:length(all_sim_fits_q)), FUN = function(x){
  stattime = as.numeric(all_sim_fits_q[[x]]$statdet_time)
  movetime = as.numeric(all_sim_fits_q[[x]]$movdet_time)
...
ggplot() +
  ...
  scale_color_discrete(name = "model", labels = c("moving detector", "stationary detector")) +
  xlab("Seconds") +
  theme_bw()
...
saveRDS(all_sim_fits_q, "writing_up/linear_all_sim_fits_q_50_fitspacing50xsimspacing.RDS")
