...
trapsdf <- rbind(
  data.frame(trapID = 1, x = seq(750, 1750, 250), y = 1000, 
             time = seq(ymd_hms("2024-01-01 8:00:00"), (ymd_hms("2024-01-01 8:00:00") + (5-1)*360), 
                        by = 360)),
  data.frame(trapID = 2, x = seq(750, 1750, 250), y = 1500, 
...
plotdat1 <- cbind(mesh, apply(as.array(0:(nrow(mesh)-1)), 1, FUN = function(meshx){haz_cpp(capthist[ex_i,ex_j], meshx, (ex_j-1), lambda0, sigma, dist_dat)}))#trap 1

colnames(plotdat1)[3] <- "hazard"
plotdat1$survival <- apply(as.array(0:(nrow(mesh)-1)), 1, FUN = function(x){surv_cpp(dist_dat$times[1],capthist[ex_i,ex_j], timeincr/100,x, (ex_j-1), lambda0, sigma, dist_dat)})
ggplot() +
  geom_raster(data = plotdat1,
              mapping = aes(x = x, y = y, fill = log(hazard))) +
  #trap 
  geom_point(data = trapsdf[trapsdf$trapID == 1 & trapsdf$time == capthist[ex_i,ex_j],], aes(x = x, y = y), shape = "+") +
  #an example simulated individual detected by this trap at this time
  geom_point(data = pop[ex_i,], aes(x = x, y = y)) +
  ggtitle(paste("log hazard for each mesh at time", capthist[ex_i,ex_j], "by trap ", ex_j))
...
ggplot() +
  ...
  #an example simulated individual detected by this trap at this time
  geom_point(data = pop[ex_i,], aes(x = x, y = y)) +
  ggtitle(paste("Survival prob for each mesh at time", capthist[ex_i,ex_j], "by trap", ex_j))

#find mesh pt closest to individual i (pop exi)
...
plotdat2 <- data.frame(times = trapsdf[trapsdf$trapID == ex_j, "time"],
                       haz = sapply(1:length(trapsdf[trapsdf$trapID == ex_j, "time"]),
                                    FUN = function(timej){
                                      ...
                                      haz_cpp(t, closestmesh_m, (ex_j-1), lambda0, sigma, dist_dat) }),
                       surv = sapply(1:length(trapsdf[trapsdf$trapID == ex_j, "time"]),
                                     FUN = function(timej){
                                       t = trapsdf[trapsdf$trapID == ex_j, "time"][timej]
                                         surv_cpp(dist_dat$times[1], t, timeincr, closestmesh_m, (ex_j-1), lambda0, sigma, dist_dat)
                                     }),
